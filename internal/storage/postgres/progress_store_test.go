...
func TestProgressStore_UpsertSiteStats(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		t.Run("insert new "+tc.statusClass, func(t *testing.T) {
			...
			case "2xx": f2 = deltaVisits
			case "3xx": f3 = deltaVisits
			case "4xx": f4 = deltaVisits
			case "5xx": f5 = deltaVisits
			}
...
func TestProgressStore_ListJobSites(t *testing.T) {
	...
	t.Run("success", func(t *testing.T) {
		rows := pgxmock.NewRows([]string{"job_id", "site", "last_update", "visits", "bytes_total", "fetch_2xx", "fetch_3xx", "fetch_4xx", "fetch_5xx"}).
